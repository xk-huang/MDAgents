...
class Agent:
    def __init__(
        self, instruction, role, examplers=None, model_info="gpt-4o-mini", img_path=None
    ):
...
class Group:
    def __init__(self, goal, members, question, examplers=None):
        ...
        for member_info in members:
            _agent = Agent(
                "You are a {} who {}.".format(
                    member_info["role"], member_info["expertise_description"].lower()
                ),
        ...
        if comm_type == "internal":
            ...
            for a_mem in assist_members:
                investigation = a_mem.chat(
                    "You are in a medical group where the goal is to {}. Your group lead is asking for the following investigations:\n{}\n\nPlease remind your expertise and return your investigation summary that contains the core information.".format(
                        self.goal, delivery
                    )
                )
            ...
            for investigation in investigations:
                gathered_investigation += "[{}]\n{}\n".format(
                    investigation[0], investigation[1]
                )
...
def parse_group_info(group_info):
    ...
    parsed_info = {"group_goal": "", "members": []}
    ...
    for line in lines[1:]:
        if line.startswith("Member"):
            ...
            member_expertise = (
                member_role_description[1].strip()
                if len(member_role_description) > 1
                else ""
            )

            parsed_info["members"].append(
                {"role": member_role, "expertise_description": member_expertise}
            )
...
def process_intermediate_query(question, examplers, model, args):
    ...
    for idx, agent in enumerate(agents_data):
        try:
            print(
                f"Agent {idx+1} ({agent_emoji[idx]} {agent[0].split('-')[0].strip()}): {agent[0].split('-')[1].strip()}"
            )
        except:
    ...
    for n in range(1, num_rounds + 1):
        ...
        agent_rs.chat(
            "You are a medical assistant who excels at summarizing and synthesizing based on multiple experts from various domain experts."
        )
                ...
                if "yes" in participate.lower().strip():
                    ...
                    for ce in chosen_experts:
                        specific_question = v.chat(
                            f"Please remind your medical expertise and then leave your opinion to an expert you chose (Agent {ce}. {medical_agents[ce-1].role}). You should deliver your opinion once you are confident enough and in a way to convince other expert with a short reason."
                        )

                        print(
                            f" Agent {idx+1} ({agent_emoji[idx]} {medical_agents[idx].role}) -> Agent {ce} ({agent_emoji[ce-1]} {medical_agents[ce-1].role}) : {specific_question}"
                        )
                        interaction_log[round_name][turn_name][f"Agent {idx+1}"][
                            f"Agent {ce}"
                        ] = specific_question
    ...
    print("\nInteraction Log")
    myTable = PrettyTable(
        [""] + [f"Agent {i+1} ({agent_emoji[i]})" for i in range(len(medical_agents))]
    )
    ...
    for i in range(1, len(medical_agents) + 1):
        ...
        for j in range(1, len(medical_agents) + 1):
            if i == j:
                ...
                i2j = any(
                    interaction_log[f"Round {k}"][f"Turn {l}"][f"Agent {i}"][
                        f"Agent {j}"
                    ]
                    is not None
                ...
                j2i = any(
                    interaction_log[f"Round {k}"][f"Turn {l}"][f"Agent {j}"][
                        f"Agent {i}"
                    ]
                    is not None
                    for k in range(1, len(interaction_log) + 1)
    ...
    _decision = moderator.temp_responses(
        f"Given each agent's final answer, please review each agent's opinion and make the final answer to the question by taking majority vote. Your answer should be like below format:\nAnswer: C) 2th pharyngeal arch\n{final_answer}\n\nQuestion: {question}",
        img_path=None,
    )
    ...
    print(
        "\U0001f468\u200d\u2696\ufe0f"
        + f"moderator's final decision (by majority vote): {_decision}"
    )
    print()
...
def process_advanced_query(question, model, args):
    ...
    for i1, gs in enumerate(group_strings):
        ...
        for i2, member in enumerate(res_gs["members"]):
            print(
                f" Member {i2+1} ({member['role']}): {member['expertise_description']}"
            )
        print()
    ...
    for group_instance in group_instances:
        if (
            "initial" in group_instance.goal.lower()
            or "iap" in group_instance.goal.lower()
        ):
    ...
    for group_instance in group_instances:
        if (
            "initial" not in group_instance.goal.lower()
            and "iap" not in group_instance.goal.lower()
        ):
    ...
    for group_instance in group_instances:
        if (
            "review" in group_instance.goal.lower()
            or "decision" in group_instance.goal.lower()
            or "frdt" in group_instance.goal.lower()
        ):
